...
void ui_app_set_date(const char *date);       // "THU 07/16" (hold lock)
void ui_app_set_wifi_status(const char *ssid, int8_t rssi, bool ok);  // local radio status
void ui_app_set_battery(int percent, bool ok, bool charging); // estimated Li-ion state of charge
